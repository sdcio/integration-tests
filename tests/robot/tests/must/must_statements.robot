...
Set system0 admin-state disable -> Fail
    ...
    ${result} =    DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=system0]/admin-state:::disable
...
Set system0 admin-state enable -> Pass
    ...
    DSCreateCandidate    ${srlinux1-name}    ${srlinux1-candidate}
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=system0]/admin-state:::enable
    Should Be Equal As Integers    ${result.rc}    0
...
Set ethernet-1/1 admin-state disable -> Pass
    ...
    DSCreateCandidate    ${srlinux1-name}    ${srlinux1-candidate}
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/admin-state:::disable
    Should Be Equal As Integers    ${result.rc}    0
...
Set lag-type without 'interface[name=xyz]/lag/lacp' existence
    ...
    DSCreateCandidate    ${srlinux1-name}    ${srlinux1-candidate}
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=lag1]/lag/lag-type:::lacp
    Should Be Equal As Integers    ${result.rc}    0
...
Set lag-type with 'interface[name=xyz]/lag/lacp' existence
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=lag1]/lag/lacp/admin-key:::1
    Should Be Equal As Integers    ${result.rc}    0
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=lag1]/lag/lag-type:::lacp
    Should Be Equal As Integers    ${result.rc}    0
...
Set auto-negotiate on non allowed interface
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-0/1]/ethernet/auto-negotiate:::true
    Should Be Equal As Integers    ${result.rc}    0
...
Set auto-negotiate on allowed interface
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/ethernet/auto-negotiate:::true
    Should Be Equal As Integers    ${result.rc}    0
...
Set auto-negotiation on breakout-mode port
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/breakout-mode/num-breakout-ports:::4
    Should Be Equal As Integers    ${result.rc}    0
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/ethernet/auto-negotiate:::true
    Should Be Equal As Integers    ${result.rc}    0
...
Set breakout-port num to 2 and port-speed to 100G
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/breakout-mode/breakout-port-speed:::25G
    Should Be Equal As Integers    ${result.rc}    0

    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/breakout-mode/num-breakout-ports:::2
    Should Be Equal As Integers    ${result.rc}    0
...
Set interface ethernet l2cp-transparency lldp tunnel true
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/ethernet/l2cp-transparency/lldp/tunnel:::true
    Should Be Equal As Integers    ${result.rc}    0
...
Set interface ethernet l2cp-transparency lldp tunnel true on lldp true interface
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    /system/lldp/interface[name=ethernet-1/1]/admin-state:::enable
    Should Be Equal As Integers    ${result.rc}    0
...
Set bfd for non existing subinterface
    ...
    ${result} =     DSSet    ${srlinux1-name}    ${srlinux1-candidate}    /bfd/subinterface/id:::ethernet-1/1.26
    Should Be Equal As Integers    ${result.rc}    0 
...
Check LAG interface member speed is set
    ...
    DSCreateCandidate    ${srlinux1-name}    ${srlinux1-candidate}
    ${result} =    DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/ethernet/aggregate-id:::lag1
    Should Be Equal As Integers    ${result.rc}    0
...
Check LAG interface vlan-tagging not set on member
    ...
    DSCreateCandidate    ${srlinux1-name}    ${srlinux1-candidate}
    ${result} =    DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/vlan-tagging:::true
    Should Be Equal As Integers    ${result.rc}    0
    ${result} =    DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/ethernet/port-speed:::10G
    Should Be Equal As Integers    ${result.rc}    0
    ${result} =    DSSet    ${srlinux1-name}    ${srlinux1-candidate}    interface[name=ethernet-1/1]/ethernet/aggregate-id:::lag1
    Should Be Equal As Integers    ${result.rc}    0
